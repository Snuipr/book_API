...
def add_book(add_body: Book.Book_add):
    try:
        conn = psycopg2.connect(dbname="API", user="postgres", password="1", host="localhost", port="5432")
        cursor = conn.cursor()
...
def delete_book(del_body: Book.Book_delete):
    try:
        ...
        if not result:
            ...
            result = cursor.fetchall()
            if result:
                cursor.execute("DELETE FROM books_text WHERE book_name = %s",
                               (del_body.book_name,))
                cursor.execute("DELETE FROM books WHERE book_name = %s",
                               (del_body.book_name,))
                cursor.close()
                conn.close()
...
def get_book_text(book_name):
    try:
        conn = psycopg2.connect(dbname="API", user="postgres", password="1", host="localhost", port="5432")
        cursor = conn.cursor()
        cursor.execute("SELECT book_text FROM books_text WHERE book_name = %s", (book_name,))
        result = cursor.fetchall()
        return result
    except Exception as error:
        raise error

def add_book_text(add_body: Book.Book_text):
    try:
        conn = psycopg2.connect(dbname="API", user="postgres", password="1", host="localhost", port="5432")
        cursor = conn.cursor()
        conn.autocommit = True
        ...
        if not result:
            ...
            result = cursor.fetchall()
            if result:
                cursor.execute("SELECT * FROM books_text WHERE book_name = %s",
                               (add_body.book_name,))
                result = cursor.fetchall()
                if not result:
                    cursor.execute("INSERT INTO books_text(book_name, user_name, book_text) VALUES(%s, %s, %s)",
                                (add_body.book_name, add_body.user_name, add_body.book_text))
                    cursor.close()
                    conn.close()
                    return f"Вы успешно добавили текст в книгу - {add_body.book_name}"
                return f"У книги - {add_body.book_name}. Уже есть текст"
            cursor.close()
            conn.close()
            return "Вы не владелец книги или такой книги не существует"
    except Exception as error:
...
def update_book_text(add_body: Book.Book_text):
    try:
        conn = psycopg2.connect(dbname="API", user="postgres", password="1", host="localhost", port="5432")
        cursor = conn.cursor()
        conn.autocommit = True
        ...
        if not result:
            ...
            if result:
                ...
                cursor.close()
                conn.close()
                return f"Вы успешно обновили текст у книги - {add_body.book_name}"
            cursor.close()
            conn.close()
            return "Вы не владелец книги или такой книги не существует"
    except Exception as error:
...
def delete_user(del_body: User):
    try:
        ...
        if not result:
            ...
            conn.close()
            return "Неверный логин или пароль"
        cursor.execute("DELETE FROM books_text WHERE user_name = %s", (del_body.user_name,))
