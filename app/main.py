...
def add_book(book_body: Book.Book_add):
    return utils_db.add_books(book_body)

@app.post("/add_book_text")
def add_book_text(add_body: Book.Book_info):
    return utils_db.add_books_info(add_body)

@app.post("/register")
def add_user(user_body: User):
    return utils_db.add_users(user_body)
